...
import 'package:flutter_module/CalculatorPageHourly.dart';
import 'package:flutter_module/CalculatorPagePerdiem.dart';
import 'package:flutter_module/CalendarPageManual.dart';
...
import 'ResultPageHourly.dart';
import 'ResultPagePerdiem.dart';

class CalendarPagePerdiem extends StatefulWidget{
   Map<DateTime,List<dynamic>> events_map;
   SharedPreferences prefs;

   CalendarPagePerdiem();
   CalendarPagePerdiem.events(this.events_map,this.prefs);
   CalendarPagePerdiem.prefs(this.prefs);
  @override
  State createState()=>(events_map==null)?_CalendarPagePerdiemState.prefs(prefs):_CalendarPagePerdiemState.events(events_map,prefs);
}

class _CalendarPagePerdiemState extends State<CalendarPagePerdiem>
{
  SharedPreferences prefs;
  Map<DateTime,List<dynamic>> events_map=null;
   SharedPreferences prefs_calculator=null;
   _CalendarPagePerdiemState.prefs(this.prefs_calculator);
   _CalendarPagePerdiemState.events(this.events_map,this.prefs_calculator);
  _CalendarPagePerdiemState();
  CalendarController _controller;
    ...
    void initState(){
      ...
      if(prefs_calculator!=null){
        ...
        setState(() {
          prefs.setString("events_perdiem", json.encode((encodeMap(_events))));
        });
      }else {
        initPrefs();
       ...
       return Scaffold(
                              ...
                              child: FlatButton(
                                ...
                                onPressed:(){
                                  Navigator.of(context).push(new MaterialPageRoute(builder:
                                      (BuildContext context) => ResultPagePerdiem()));
                                } ,
                            )
                          )
                      ),
        ...
        floatingActionButton: FloatingActionButton(
          ...
          onPressed: (){
            ...
            print(prefs.getString("perdiem"));
            print("CalculatorPage");
            Navigator.of(context).push(new MaterialPageRoute(builder:
                (BuildContext context) => CalculatorPagePerdiem(
